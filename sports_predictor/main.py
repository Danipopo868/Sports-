    ...
    def scan(self, date_iso: str) -> dict[str, dict[str, Any]]:
        ...
        for sport in SPORTS:
            try:
                ...
                recommendation, best_observed, notes = analyze_sport(
                    sport,
                    games,
                    quotes,
                    forms,
...
def run(args: argparse.Namespace) -> int:
    ...
    while not stop_event.is_set():
        ...
        print(
            f"[{now.isoformat()}] Escaneo #{scan_number} de {date_iso}",
            flush=True,
        )
        ...
        for sport in SPORTS:
            ...
            if recommendation:
                ...
                print(
                    f"{sport}: APOSTAR {recommendation.selection}",
                    flush=True,
                )
                ...
                if best_observed:
                    print(
                        f"  Mejor opción observada: {best_observed}",
                        flush=True,
                    )
        ...
        print(
            f"Reporte actualizado: {latest_md}",
            flush=True,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Analiza MLB, NFL y NBA sin conectar "
            "con ninguna plataforma de apuestas."
        )
    )
